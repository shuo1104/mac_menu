...
struct MusicControllerSelectionView: View {
    ...
    @State private var selectedMediaController: MediaControllerType =
        MediaControllerType.resolved(Defaults[.mediaController])
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Button("Continue", action: {
                let choice = MediaControllerType.resolved(
                    self.selectedMediaController
                )
                self.mediaController = choice
            ...
            .controlSize(.large)
            .disabled(
                !availableMediaControllers.contains(selectedMediaController)
            )
            .padding(.bottom, 24)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        .onAppear {
            selectedMediaController = MediaControllerType.resolved(
                selectedMediaController
            )
        }
        .background(
