...
private struct ScrollMonitor: NSViewRepresentable {
    ...
    @MainActor final class Coordinator: NSObject {
        ...
        private func handleScroll(_ event: NSEvent) {
            // Ignore scrolls aimed at a scrollable view (e.g. the usage detail
            // list) so scrolling its content doesn't trigger the notch gesture.
            guard !isOverScrollableContent(event) else { return }
        ...
        /// Returns true when the pointer is over an `NSScrollView` (including
        /// SwiftUI `ScrollView`s, which are backed by one) that can actually
        /// scroll along this gesture's axis.
        private func isOverScrollableContent(_ event: NSEvent) -> Bool {
            ...
            var hit: NSView? = contentView.hitTest(point)
            while let view = hit {
                if let scrollView = view as? NSScrollView,
                   canScroll(scrollView, horizontal: direction.isHorizontal) {
                    return true
                }
                hit = view.superview
            }
        ...
        private func canScroll(_ scrollView: NSScrollView, horizontal: Bool) -> Bool {
            guard let document = scrollView.documentView else { return false }
            let visible = scrollView.contentView.bounds.size
            let extent = document.frame.size
            return horizontal
                ? extent.width > visible.width + 1
                : extent.height > visible.height + 1
        }
    }
}
