...
class MusicManager: ObservableObject {
    ...
    /// Becomes true after the async deprecation probe finishes (success or failure).
    public private(set) var didFinishDeprecationCheck: Bool = false
    ...
    @ObservedObject var coordinator = MMViewCoordinator.shared
    ...
    init() {
        ...
        Task { @MainActor in
            ...
            self.didFinishDeprecationCheck = true
    ...
    private func setActiveControllerBasedOnPreference() {
        ...
        let controllerType = MediaControllerType.resolved(preferredType)
        print("Preferred Media Controller: \(preferredType) -> \(controllerType)")

        if preferredType != controllerType {
    ...
    private func updateSneakPeek() {
        if isPlaying && Defaults[.enableSneakPeek] {
            if Defaults[.sneakPeekStyles] == .standard {
                ...
                coordinator.toggleExpandingView(status: true)
            }
        }
    }
