...
struct WidgetData: Hashable {
    ...
    /// Names of models this period's spend used that the pricing sources can't price. Drives the label
    /// warning triangle and its hover list. Token-first local-log providers retain their measured tokens
    /// with an unknown cost; other sources may omit rows they cannot account for. Empty for every other row.
    ...
    var boundedHeadline: String {
        ...
        let localizedMode = L10n.string(displayMode.label)
        let modeWord = localizedMode == displayMode.label ? localizedMode.lowercased() : localizedMode
        return "\(valueText) \(modeWord)"
    }
    ...
    var boundedSubtitle: String? {
        ...
        if let periodDurationMs,
           let duration = Formatters.compactDuration(TimeInterval(periodDurationMs) / 1000) {
            return String(format: L10n.string("Resets in %@"), duration)
        }
        switch kind {
        case .percent:
            return nil
        case .dollars:
            ...
            return "\(amount) \(L10n.string(limitNoun ?? "limit"))"
        case .count:
            // The unit (e.g. "credits") shows whether the count is bounded or a plain balance.
            return countSuffix.map { L10n.string($0) }
        }
    }
    ...
    var unboundedDetail: String {
        ...
        if !selected.isEmpty {
            ...
            if selected.count == 1 {
                ...
                if value.kind == .dollars, let word = unboundedValueWord {
                    return "\(MetricFormatter.number(value.number, kind: .dollars, style: .row)) \(L10n.string(word))"
                }
        ...
        let sourceWord = unboundedValueWord ?? displayMode.label
        let localizedWord = L10n.string(sourceWord)
        let word = localizedWord == sourceWord ? localizedWord.lowercased() : localizedWord
        if kind == .count, let countSuffix {
            return "\(valueText) \(L10n.string(countSuffix)) \(word)"
        }
    ...
    /// True when this period used at least one model the pricing manifest can't price, so no complete
    /// dollar figure is available. Drives the label warning triangle on spend/token tiles.
    ...
    var unknownModelTooltip: String? {
        ...
        let header = L10n.string(unknownModels.count == 1 ? "Unknown model found" : "Unknown models found")
