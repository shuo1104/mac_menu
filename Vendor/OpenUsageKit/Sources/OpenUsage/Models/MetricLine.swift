...
enum MetricLine: Hashable, Sendable, Codable {
    ...
    /// don't know. The row shows a warning triangle listing them on hover; token-first local-log
    /// providers retain their measured tokens while leaving cost unknown, while other sources may omit
    /// rows they cannot account for coherently. `modelBreakdown` carries the period-scoped ranked model
