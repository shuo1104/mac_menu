...
struct TotalSpendCard: View {
    ...
    @AppStorage(UsagePeriod.storageKey) private var periodRawValue = TotalSpendPeriod.today.rawValue
    /// The selected metric (Cost / Cost/MTok / Tokens) survives the same way.
    @AppStorage("openusage.totalSpend.metric") private var metricRawValue = TotalSpendMetric.tokens.rawValue
    ...
    private var metric: TotalSpendMetric {
        TotalSpendMetric(rawValue: metricRawValue) ?? .tokens
    }
            ...
            ForEach(TotalSpendMetric.allCases) { option in
                Button {
                    ...
                    if option == metric {
                        ...
                        Text(verbatim: L10n.string(option.title))
                    }
                }
            }
        } label: {
    ...
    private var infoTooltip: String {
        ...
        return String(
            format: L10n.string("Only includes %@."),
    ...
    private var emptyState: some View {
        Text(verbatim: L10n.string(metric.emptyMessage))
...
struct TotalSpendRingContent: View {
    ...
    private var centerLabel: some View {
        ...
        return VStack(spacing: 1) {
            ...
            Text(verbatim: L10n.string(center.unit))
