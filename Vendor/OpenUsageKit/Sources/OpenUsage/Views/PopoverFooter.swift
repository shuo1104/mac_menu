...
struct PopoverFooter: View {
    ...
    private var footerIdentity: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let notice = layout.pinLimitNotice {
                Text(verbatim: L10n.string(notice))
    ...
    private var nextUpdateButton: some View {
        Button {
            ...
            TimelineView(.periodic(from: .now, by: 1)) { context in
                HStack(spacing: 5) {
                    Text(verbatim: updateStatusText(now: context.date))
                        .monospacedDigit()
    ...
    private func updateStatusText(now: Date) -> String {
        if isUpdating { return L10n.string("Updating…") }
        ...
        if totalSeconds >= 60 {
            ...
            return String(format: L10n.string("Next update in %@"), "\(minutes)m")
        }
        return String(format: L10n.string("Next update in %@"), "\(totalSeconds)s")
    }
}
