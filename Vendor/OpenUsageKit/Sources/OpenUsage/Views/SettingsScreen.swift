...
struct SettingsScreen: View {
    ...
    private var content: some View {
        ...
        return VStack(alignment: .leading, spacing: density.sectionSpacing) {
            section("General") {
                ...
                row("Show Model Usage") {
                    Toggle("", isOn: $showModelUsage)
                        .settingsSwitchStyle()
                }
                row("Launch at Login") {
                    Toggle("", isOn: Binding(
    ...
    private func notifToggleRow(_ milestone: PaceMilestone, isOn: Binding<Bool>) -> some View {
        HStack(spacing: 6) {
            Text(verbatim: L10n.string(milestone.settingLabel))
            Image(systemName: "info.circle")
                .imageScale(.small)
                .foregroundStyle(.secondary)
                .hoverTooltip(L10n.string(milestone.tooltip))
            Spacer(minLength: 8)
            Toggle("", isOn: isOn)
                .settingsSwitchStyle()
        }
        ...
        VStack(spacing: 0) {
            ...
            Button {
                ...
                Text(verbatim: L10n.string(
                    notificationsAuth == .denied ? "Open System Settings" : "Allow Notifications"
                ))
    ...
    private func logButton(_ title: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(verbatim: L10n.string(title)).frame(maxWidth: .infinity)
        }
        .glassButtonStyle()
        .controlSize(.regular)
        .padding(.horizontal, 12)
    ...
    private func row(_ label: String, @ViewBuilder control: () -> some View) -> some View {
        HStack(spacing: 10) {
            Text(verbatim: L10n.string(label))
            Spacer(minLength: 8)
            control()
        }
        .padding(.horizontal, 12)
    ...
    private func picker<Value: Hashable>(
        ...
        Picker("", selection: selection) {
            ForEach(options, id: \.self) { option in
                Text(verbatim: L10n.string(label(option))).tag(option)
            }
        }
        .pickerStyle(.menu)
        .labelsHidden()
