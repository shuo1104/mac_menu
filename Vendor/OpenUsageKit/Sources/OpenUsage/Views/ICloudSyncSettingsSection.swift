...
struct ICloudSyncSettingsSection: View {
    ...
    private func relativeAge(_ date: Date, now: Date) -> String {
        ...
        if seconds < 3_600 {
            return String(format: L10n.string("%@ ago"), "\(max(1, Int(seconds / 60)))m")
        }
        if seconds < 86_400 {
            return String(format: L10n.string("%@ ago"), "\(max(1, Int(seconds / 3_600)))h")
        }
        return String(format: L10n.string("%@ ago"), "\(max(1, Int(seconds / 86_400)))d")
    }
}
