...
struct AntigravityUsageClient: Sendable {
    ...
    var http: HTTPClient
    var oauthCredentials: AntigravityOAuthCredentials?

    init(
        lsHTTP: HTTPClient = URLSessionHTTPClient(allowsInsecureLoopback: true),
        http: HTTPClient = URLSessionHTTPClient(),
        oauthCredentials: AntigravityOAuthCredentials? = nil
    ) {
        self.lsHTTP = lsHTTP
        self.http = http
    ...
    func refreshGoogleToken(_ refreshToken: String) async -> TokenRefreshOutcome {
        guard let oauthCredentials else { return .notConfigured }
        guard let url = URL(string: Self.googleOAuthURL) else { return .unavailable }
        let form = [
            "client_id=\(oauthCredentials.clientID.urlFormEncoded)",
            "client_secret=\(oauthCredentials.clientSecret.urlFormEncoded)",
