...
final class ClaudeProvider: ProviderRuntime {
    ...
    private func localUsageSnapshot() async -> ProviderSnapshot? {
        ...
        if snapshot != nil {
            AppLog.info(LogTag.plugin("claude"), "no usable Claude account; showing local token history only")
        }
        return snapshot
    }
    ...
    private func probe(
        ...
        // shared pricing store. The scan runs on its scanner actor, off the main actor.
        let pricing = await pricing()
        let nativeScan = await logUsageScanner.scan(now: now(), pricing: pricing)
        var usageHistory: ProviderUsageHistory?
        if !Task.isCancelled, let scan = nativeScan {
            let note = "From your Claude usage history (estimated)"
