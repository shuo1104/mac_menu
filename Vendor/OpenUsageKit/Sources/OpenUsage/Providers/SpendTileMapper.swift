...
enum SpendTileMapper {
    ...
    static func appendTokenUsage(
        ...
        let usedEntries = usage.daily.filter { hasUsage($0) }
        let hasUnpricedUsage = usedEntries.contains { $0.costUSD == nil && $0.totalTokens > 0 }
        let costSamples = usedEntries.compactMap(\.costUSD)
        let totalCost = hasUnpricedUsage || costSamples.isEmpty ? nil : costSamples.reduce(0, +)
