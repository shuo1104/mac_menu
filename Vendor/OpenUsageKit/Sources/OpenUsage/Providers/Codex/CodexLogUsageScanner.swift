...
actor CodexLogUsageScanner {
    ...
    /// Cheap local-only capability probe used to enable Codex for API-key/custom-gateway setups that
    /// have rollout logs but no ChatGPT OAuth token for the subscription usage endpoint.
    func hasUsageLogs() -> Bool {
        !Self.sessionFiles(homes: codexHomes()).isEmpty
    }
    ...
    ///
    /// Events that can't be priced still contribute their measured tokens. Their cost remains nil.
    /// A named unknown model also lands in `unknownModelsByDay`; a blank slug is grouped as unattributed.
        ...
        for event in events where event.timestamp >= since {
            ...
            guard let model = trimmedModel else {
                accumulator.add(
                    ...
                    tokens: event.total,
                    cost: nil,
                    model: ModelUsageEntry.unattributedModelName
                )
                continue
            }
            ...
            guard let rates = resolvedRates else {
                ...
                accumulator.add(day: day, tokens: event.total, cost: nil, model: model)
                continue
            }
