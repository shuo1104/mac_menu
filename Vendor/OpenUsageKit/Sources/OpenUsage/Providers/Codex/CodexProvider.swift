...
final class CodexProvider: ProviderRuntime {
    ...
    func hasLocalCredentials() async -> Bool {
        if await logUsageScanner.hasUsageLogs() { return true }
    ...
    func refresh() async -> ProviderSnapshot {
        ...
        for candidate in fileCandidates {
            do {
                ...
                continue
            } catch let error as CodexAuthError where error == .usageAPIKey {
    ...
    private func localUsageSnapshot() async -> ProviderSnapshot? {
        ...
        if snapshot != nil {
            AppLog.info(LogTag.plugin("codex"), "no usable ChatGPT account; showing local token history only")
        }
        return snapshot
    }
    ...
    private func probe(authState initialState: CodexAuthState) async throws -> ProviderSnapshot {
        ...
        // the shared pricing store. The scan runs on its scanner actor, off the main actor.
        let pricing = await pricing()
        let nativeScan = await logUsageScanner.scan(now: now(), pricing: pricing)
        var usageHistory: ProviderUsageHistory?
        if !Task.isCancelled, let scan = nativeScan {
            let note = "From your Codex logs (estimated)"
