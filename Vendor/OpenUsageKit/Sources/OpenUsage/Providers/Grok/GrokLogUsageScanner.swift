...
struct GrokLogUsageScanner: Sendable {
    ...
    /// Local-only capability probe used by provider auto-detection.
    func hasUsageLog() -> Bool {
    ...
    static func parse(_ text: String, since: Date, pricing: ModelPricing) -> LogUsageScan {
        ...
        text.enumerateLines { line, _ in
            ...
            // Grok's token rows lack a model id; attribute via the row's process. Tokens remain useful
            // even when attribution or pricing is unavailable, so those rows stay in the daily totals
            // with a nil cost.
            ...
            let cost = pricing.estimatedCostDollars(model: model, tokens: tokenBreakdown)
            if cost == nil {
                if totalTokens > 0 {
    ...
    private static func modelID(msg: String, ctx: [String: Any]) -> String? {
        ...
        case "backend_search: model switch":
            raw = ctx["new_model"] ?? ctx["model"] ?? ctx["current_model_id"] ?? ctx["model_id"]
        case "subagent model resolved":
            raw = ctx["child_model"] ?? ctx["model_id"] ?? ctx["model"]
        default:
            return nil
        }
