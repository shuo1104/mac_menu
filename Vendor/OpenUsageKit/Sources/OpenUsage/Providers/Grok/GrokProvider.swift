...
final class GrokProvider: ProviderRuntime {
    ...
    func hasLocalCredentials() async -> Bool {
        if logUsageScanner.hasUsageLog() { return true }
        // Same remote source as `refresh()`: ~/.grok/auth.json with at least one keyed entry.
    ...
    func refresh() async -> ProviderSnapshot {
        ...
        } catch let error as GrokAuthError where error == .notLoggedIn || error == .invalidAuth {
            if let local = await localUsageSnapshot() { return local }
    ...
    private func localUsageSnapshot() async -> ProviderSnapshot? {
        let now = now()
        let scan = await logUsageScanner.scan(daysBack: 30, now: now, pricing: await pricing())
        let snapshot = LocalLogUsageSnapshot.make(
            provider: provider,
            scan: scan,
            now: now,
            sourceNote: "From your Grok logs (estimated)"
        ...
        if snapshot != nil {
            AppLog.info(LogTag.plugin("grok"), "no usable Grok account; showing local token history only")
        }
        return snapshot
    }
